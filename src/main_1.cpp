    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          * Both are in between [-1, 1].
          * Simulator has 100ms latency.
          */

          // Calculate fits of 3rd-order polynomial to the above x and y coordinates
          auto coeffs = polyfit(ptsx_car, ptsy_car, 3);

          // Calculates the cross track error
          // Points are transformed to vehicle coordinates, thst's why x = 0 and y = 0.
          ...
          double pred_px = 0.0 + v * time_lag;
          const double pred_py = 0.0;
          double pred_psi = 0.0 + (v * -delta * time_lag )/ Lf;
          double pred_v = v + at;
          double pred_cte = cte + v * sin(epsi) * time_lag;
          double pred_epsi = epsi + (v * -delta * time_lag) / Lf;
          ...
          for (std::size_t i = 2; i < vars.size(); i+=2) {
