...
// Set desired speed for the cost function (maximum speed)
const double ref_v = 50;

// The solver takes all the state variables and actuator
// variables in a singular vector. That's why, we should to find out
// starts and ends of variables.
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Weights for each cost. We can tune each parameter
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // State vector contains all current values that we need for vars
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
  ...
  // Setting the number of model variables.
